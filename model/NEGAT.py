...
from .NEGAT_layers import TimeBlock
from .NEGAT_gat import TalkingHeadLayer
...
class NEGAT(nn.Module):
    def __init__(self, cuda, adj, num_nodes, num_features, num_timesteps_input,
        ...
        self.end_conv = nn.Sequential(
            ...
            nn.GELU(),
            weight_norm(nn.Linear(512, num_timesteps_output)),
        )
    ...
    def calLoss(self, x, A, a=1, b=1):
        ...
        if self.training:
            ...
            FA1=torch.sqrt(torch.sum(A1 * A1))
...
def make_negat_model(device, num_nodes, num_features, num_timesteps_input,
                     num_timesteps_output, adj_mx, nheads=6, nhid=48):
    """
    Create NEGAT model for traffic prediction
    """
    # Generate spatial embeddings (SE) - simplified version
    SE = torch.randn(num_nodes, 64).to(device)
